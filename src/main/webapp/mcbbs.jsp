			...
			<ul class="nav navbar-nav">
				...
				<li class="active" class="dropdown"><a href="#" class="dropdown-toggle" 
					...
					<ul	class="dropdown-menu">
						<li class="active"><a href="mcbbs.jsp">기계공학부</a></li>
	...
	<div class="container"">
		<div class="row">
			<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
				...
				<tbody>
					<%
						...
						ArrayList<Bbs> list = bbsDAO.getList(pageNumber, "mcbbs");
						for(int i =0; i< list.size(); i++){
					%>
					<tr>
						<td><%= list.get(i).getBbsID() %>
						<td><a href="view.jsp?bbsID=<%= list.get(i).getBbsID() %>&userJob=기계공학부"> <%= list.get(i).getBbsTitle() %></a>
			...
			<%
				if(pageNumber !=1){
			%>
				<a href="mcbbs.jsp?pageNumber=<%=pageNumber - 1%>" class="btn btn-success btn-arraw-left">이전</a>
			<%
				}if(bbsDAO.nextPage(pageNumber + 1, "mcbbs")){
			%>	
				<a href="mcbbs.jsp?pageNumber=<%=pageNumber + 1%>" class="btn btn-success btn-arraw-left">다음</a>
			<%
				}
			%>	
