...
<body>
	<%
		...
		if(job.equals("MCBBS")) {
			userJob = "기계공학부";
		}else if(job.equals("IFBBS")) {
			userJob = "정보산업공학부";
		}else if(job.equals("CPBBS")) {
			userJob = "컴퓨터정보공학부";
		}else if(job.equals("CTBBS")) {
			userJob = "건축학부";
		}else if(job.equals("SVBBS")) {
			userJob = "서비스학부";
		}else {
			userJob="자유게시판";
		}
			...
			<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
				...
				<tbody>
					<%
						...
						ArrayList<Bbs> list = bbsDAO.getSearchList(job, searchType, searchText);
						for(int i =0; i< list.size(); i++){
					%>
					<tr>
						<td><%= list.get(i).getBbsID() %>
						<td><a href="view.jsp?bbsID=<%= list.get(i).getBbsID() %>&userJob=<%=userJob%>"> <%= list.get(i).getBbsTitle() %></a>
