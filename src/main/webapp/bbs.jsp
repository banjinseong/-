...
<body>
	...
	<div class="container"">
		<div class="row">
			<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
				...
				<tbody>
					<%
						...
						ArrayList<Bbs> list = bbsDAO.getList(pageNumber, job);
						for(int i =0; i< list.size(); i++){
					%>
					<tr>
						<td><%= list.get(i).getBbsID() %>
						<td><a href="view.jsp?bbsID=<%= list.get(i).getBbsID() %>&userJob=자유게시판"> <%= list.get(i).getBbsTitle() %></a>
			...
			<%
				}if(bbsDAO.nextPage(pageNumber + 1, job)){
			%>	
